 ...
 *
 * Server-sent events carrying live incident activity. Every listener receives
 * everything — ATLAS is the control-room view, so there is no geofencing here.
 *
 * @example
 * const stream = new EventSource('http://localhost:4102/api/events')
 * stream.addEventListener('point.created', (e) => console.log(JSON.parse(e.data)))
 */
